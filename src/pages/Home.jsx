      ...
      <div className="absolute inset-0 z-0 h-full w-full px-5 py-24 [background:radial-gradient(125%_125%_at_50%_10%,#fff_40%,#00a_100%)] dark:[background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#00a_100%)]"></div>
        <div className = "absolute top-48 h-fit">
          <div className="md:w-4/5 p-4 mx-auto bg-slate-300 dark:bg-slate-800 dark:bg-opacity-50 bg-opacity-50 space-y-5 rounded-3xl">
            <h1 className="md:text-8xl font-extrabold">Welcome to Jarilo.dev</h1>
            <p className="text-md lg:text-2xl lg:w-1/2 lg:flex-col item-center justify-center mx-auto">
              This web application serves as a data hub for users to plan and compare between various assets in the online game of Genshin Impact and Honkai Star Rails.
          ...
          <div className="md:w-4/5 p-4 mx-auto">
            <div className="grid grid-cols-2 p-2 my-4 gap-5">
              ...
              <button className="group relative transition box-content h-20  md:h-52 rounded-lg border-2 border-black dark:border-slate-800 bg-slate-100 dark:bg-slate-500" onClick = {() => {
                ...
                <h2 className="md:text-2xl lg:text-4xl md:font-bold">Light Cones</h2>
                <p className="p-4 md:text-sm lg:text-xl max-md:hidden">
                  A grid of Light Cones with filter tools to find your ideal one.
                  Select your desired Light Cone to view detailed information including rarity and location of the weapon.
                </p>
                <div className="absolute inset-0 h-full w-0 bg-gray-600 bg-opacity-30 dark:bg-white/30 transition-[width] group-hover:w-full"></div>              
              </button>
              <button className="group relative transition box-content h-20  md:h-52 rounded-lg border-2 border-black dark:border-slate-800 bg-slate-100 dark:bg-slate-500" onClick = {() => {
